  ...
  return (
    <main className="mx-auto w-full max-w-6xl flex-1 px-4 py-8 sm:px-6">
      <PageHeader
        ...
        description="Add agents, wire them together, and save the graph — running it is next up."
      >
        <Link
          href="/workflows"
      ...
      <div className="mt-6 flex flex-col gap-3 rounded-lg border border-hairline p-4 transition-colors duration-200 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="flex shrink-0 flex-col items-end gap-2">
          <div className="flex items-center gap-2">
            {/* TODO(chunk 7): wire to POST /api/v1/workflows/{id}/validate */}
            <button
              type="button"
              disabled
              title="Coming in chunk 7"
              className="cursor-not-allowed rounded-md border border-hairline px-3 py-2 text-xs text-muted opacity-40"
            >
              Validate
            </button>
            {/* TODO(chunk 7): wire to POST /api/v1/workflows/{id}/run */}
            <button
              type="button"
              disabled
              title="Coming in chunk 7"
