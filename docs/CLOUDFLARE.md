...
Face Spaces (API) + OpenNext Worker **`agentfleet-app`** (web). Do **not**
use `agentfleet.pages.dev` or `agentfleet.vercel.app` — those are not this
project. This file is an **additional** path that runs the API as a
Cloudflare Container and the web as that same OpenNext Worker.
...
cd apps/web
npm install
# Do NOT set NEXT_PUBLIC_API_URL — it is inlined at build time and falling
# back to localhost is how a hosted demo ends up Chat-only. Browser traffic
# uses same-origin /backend; only INTERNAL_API_URL points at the API Worker.
export INTERNAL_API_URL=https://agentfleet-api.<your-subdomain>.workers.dev
...
| `AUTH_URL` | wrangler `vars` | Public web URL |
| `AUTH_TRUST_HOST` | wrangler `vars` | `true` on workers.dev |
| `DEMO_LOGIN_ENABLED` | wrangler `vars` | `1` |
| `INTERNAL_API_URL` | wrangler `vars` (runtime) | Server components **and** the `/backend` proxy |
| `PUBLIC_API_URL` | optional, runtime | Only if the browser must call the API host directly |
| `NEXT_PUBLIC_SITE_URL` | wrangler `vars` | `metadataBase` / OG cards |
| `AUTH_GOOGLE_ID` / `AUTH_GOOGLE_SECRET` | optional | Demo login works without Google |

**Do not set `NEXT_PUBLIC_API_URL`.** The browser default is same-origin
`/backend` (`apps/web/app/backend/[...path]/route.ts`), which the web
Worker proxies to `INTERNAL_API_URL` at request time. That is what makes
missions / builder / evals / documents work after demo login — Chat is an
RSC and can use `INTERNAL_API_URL` directly; every other app page is a
client component that used to bake `localhost:8000` at build time.

`CORS_ORIGINS` on the API is still required for MCP clients and anything
that calls the API host directly. The web UI does not need it when it uses
`/backend`.
...
1. API `CORS_ORIGINS` = the web origin from step 4 (for non-browser
   clients). Update `deploy/cloudflare/wrangler.jsonc` `vars` and
...
After **any** API image change, rebuild/redeploy the Container — the Worker
proxy is not enough. `RUN_MIGRATIONS_ON_BOOT=1` is already a wrangler var;
the image must actually honor it (`scripts.space_boot`).
...
- [ ] Landing page loads on the `agentfleet-app` URL — **not**
...
- [ ] `/workflows` — visual builder loads; save a graph.
- [ ] `/missions` — seeded missions, start a goal, approval gate works.
- [ ] `/documents` — upload a small text file, status becomes `ready`.
- [ ] `/evals` — cases list; Run eval on a seeded agent.
- [ ] `/usage` — non-zero figures after the chat/eval above.
- [ ] Agent MCP servers field accepts a URL and persists (sandbox agent).
- [ ] Browser console: no CORS errors, no calls to `localhost:8000`.
- [ ] From a machine with `AUTH_SECRET`:
      `cd apps/api && AUTH_SECRET=… uv run python -m scripts.demo_smoke --base-url https://agentfleet-api.<subdomain>.workers.dev --require-seeded`

Do **not** mark `fleet.agentic-systems-studio.com` (or any studio subdomain)
ready until that checklist is green on the live URLs.
...
## Fallback: OpenNext frontend + HF Spaces API

If Containers cannot deploy, you can still ship a usable demo:

1. Keep the Space from [DEPLOY.md](DEPLOY.md) as the API.
2. Set web `INTERNAL_API_URL` to the Space URL. Leave `PUBLIC_API_URL` unset
   so the browser uses `/backend`.
3. Deploy only `agentfleet-app` (OpenNext).
4. Space `CORS_ORIGINS` can stay empty for the UI; set it if MCP/clients
   call the Space URL directly.
...
| `apps/api/app/database_url.py` | DSN normalize, `DATABASE_SCHEMA` search_path, `DATABASE_SSL` |
| `apps/api/tests/test_database_url.py` | Offline DSN / TLS connect-arg tests |
| `apps/api/scripts/space_boot.py` | Single-instance migrate + demo seed (`RUN_MIGRATIONS_ON_BOOT=1`) |
| `apps/api/scripts/demo_smoke.py` | Fail if builder / missions / ops are missing |
| `apps/web/wrangler.jsonc` | OpenNext Worker `agentfleet-app` |
| `apps/web/open-next.config.ts` | OpenNext adapter |
| `docs/DEPLOY.md` | Owned demo env + smoke (Spaces + `agentfleet-app`, not Vercel) |
