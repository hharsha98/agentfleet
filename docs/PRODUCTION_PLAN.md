...
- **NEXT: rest of Phase 10** — I-webhooks (inbound HMAC-verified trigger endpoint that creates a run, `webhooks` table + /automations UI section) → J (artifacts panel: sandboxed markdown/code/Vega-Lite render) → K (⌘K palette + empty states + changelog) → L (prompt playground + A/B) → M (2nd-SDK / Pydantic AI agent) → N (voice, Vapi). Each degrades gracefully without its key.
