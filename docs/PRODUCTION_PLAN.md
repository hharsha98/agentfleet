...
- **NEXT: Phase 11 remainder** — P (Sentry backend+frontend, request-ID tagged, degrades gracefully without SENTRY_DSN) → load test (nice-15, k6/locust + results note). Then Phase 12 (B auth+ownership incl. Conversation cascade fix, C rate limiting, F2 deploy-safety) → Phase 13 deploy LAST.
