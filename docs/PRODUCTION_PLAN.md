...
- **NEXT: rest of Phase 10** — J (artifacts panel: sandboxed markdown/code/Vega-Lite render) → K (⌘K palette + empty states + changelog) → L (prompt playground + A/B) → M (2nd-SDK / Pydantic AI agent) → N (voice, Vapi). Each degrades gracefully without its key.
